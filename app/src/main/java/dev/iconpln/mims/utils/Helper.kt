...
object Helper {
    ...
    fun generateReportId(prefix: String, surveyorId: Int, storeId: Int, referenceId: Int): String {
        return "$prefix.$surveyorId.$storeId.$referenceId.${
            DateTime.now().toString("yyMMddHHmmssSS")
        }"
    }
    ...
    fun getDistance(
        ...
        val a =
            Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(Math.toRadians(latitude1)) * Math.cos(
    ...
    fun checkEmailValidity(emailFormat: AppCompatEditText): Boolean {
        ...
        val getResult =
            !TextUtils.isEmpty(getEmail) && Patterns.EMAIL_ADDRESS.matcher(getEmail).matches()
    ...
    fun logout(context: Context, isLoginSso: Int) {
        val isLoginBiometric =
            SharedPrefsUtils.getIntegerPreference(context, Config.KEY_IS_LOGIN_BIOMETRIC, 0)
