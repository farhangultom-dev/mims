...
object DataController {
    ...
    fun getDirectory(context: Context?, directory: FileType): String {
        if (context != null) {
            return when (directory) {
                ...
                DataController.FileType.DATABASE_REPORT_FILE -> StorageUtils.getDirectory(
                    StorageUtils.DIRECTORY_ROOT
                ) + "/database_report.db"
    ...
    fun deleteFilesNotInReport(path: String, context: Context) {
        ...
        if (dir.exists() && dir.isDirectory) {
            ...
            val dbReport =
                dev.iconpln.mims.data.local.database_local.DatabaseReport.getDatabase(context)
    ...
    fun readpdf(c: Activity, title: String) {
        val pdfFile =
            File(getDirectory(DataController.FileType.PDF_DIRECTORY) + "/" + title + ".pdf")
        if (pdfFile.exists()) {
