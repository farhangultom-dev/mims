...
class ApprovalMaterialViewModel(private val apiService: ApiService) : ViewModel() {
    ...
    val getMaterialAktivasiResponse: LiveData<GetMaterialAktivasiResponse?> =
        _getMaterialAktivResponse
    ...
    private val _detailMaterialRegistrasiResponse =
        MutableLiveData<GetMaterialRegistrasiDetailByDateResponse?>()
    val detailMaterialRegistrasiResponse: LiveData<GetMaterialRegistrasiDetailByDateResponse?> =
        _detailMaterialRegistrasiResponse

    private val _nomorMaterialResponse = MutableLiveData<GetNomorMaterialForAktivasiResponse?>()
    val nomorMaterialResponse: LiveData<GetNomorMaterialForAktivasiResponse?> =
        _nomorMaterialResponse
    ...
    fun getMaterialRegistrasiDetailByDate(
        ...
        _isLoading.value = true
        job = CoroutineScope(Dispatchers.IO + exceptionHandler).launch {
            val response = apiService.getMaterialRegistrasiDetailByDate(
                tgl_registrasi,
                status,
                sn,
                no_material
