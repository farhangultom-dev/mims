...
class PenerimaanUlpActivity : BaseActivity() {
    ...
    private lateinit var penerimaanUlps: List<dev.iconpln.mims.data.local.database.TTransPenerimaanUlp>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        penerimaanUlps = daoSession.tTransPenerimaanUlpDao.queryBuilder()
            .whereOr(
                dev.iconpln.mims.data.local.database.TTransPenerimaanUlpDao.Properties.StatusPenerimaan.notEq(
                    "DITERIMA"
                ),
                dev.iconpln.mims.data.local.database.TTransPenerimaanUlpDao.Properties.StatusPemeriksaan.notEq(
                    "SUDAH DIPERIKSA"
                )
            ).list()
        ...
        adapter =
            PenerimaanULPAdapter(arrayListOf(), object : PenerimaanULPAdapter.OnAdapterListener {
                override fun onClick(pengujian: dev.iconpln.mims.data.local.database.TTransPenerimaanUlp) {
                    insertDataDetail(pengujian.noRepackaging)
                    if (pengujian.isDonePemeriksaan == 1) {
                        Toast.makeText(
                            this@PenerimaanUlpActivity,
                            "Anda sudah menyelesaikan pengiriman ini",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else if (pengujian.statusPemeriksaan == "SUDAH DIPERIKSA" && pengujian.statusPenerimaan == "DITERIMA") {
                        Toast.makeText(
                            this@PenerimaanUlpActivity,
                            "Pengiriman ini sudah di selesaikan",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        startActivity(
                            Intent(
                                this@PenerimaanUlpActivity,
                                PetugasPemeriksaanActivity::class.java
                            )
        ...
        with(binding) {
            ...
            rvPenerimaanULP.layoutManager =
                LinearLayoutManager(this@PenerimaanUlpActivity, LinearLayoutManager.VERTICAL, false)
        }
        ...
        binding.srcNoPengirimanULP.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(s: Editable?) {
                ...
                val searchList = daoSession.tTransPenerimaanUlpDao.queryBuilder()
                    .where(
                        dev.iconpln.mims.data.local.database.TTransPenerimaanUlpDao.Properties.DeliveryDate.like(
                            "%" + deliveryDate + "%"
                        )
                    )
                    .where(
                        dev.iconpln.mims.data.local.database.TTransPenerimaanUlpDao.Properties.NoPengiriman.like(
                            "%" + srcNoPengiriman + "%"
                        )
                    )
                    .list()
        ...
        binding.srcNoPermintaanULP.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(sp: Editable?) {
                ...
                val searchPermintaanList = daoSession.tTransPenerimaanUlpDao.queryBuilder()
                    .where(
                        dev.iconpln.mims.data.local.database.TTransPenerimaanUlpDao.Properties.NoPermintaan.like(
                            "%" + srcNoPermintaan + "%"
                        )
                    )
                    .list()
    ...
    private fun setDatePicker() {
        ...
        binding.edtTanggalDiterimaULP.setOnClickListener {
            DatePickerDialog(
                this@PenerimaanUlpActivity, dateSetListenerPermintaan,
    ...
    private fun insertDataDetail(noRepackaging: String) {
        ...
        val penerimaanDetails = daoSession.tPenerimaanDetailUlpDao.queryBuilder()
            .where(
                dev.iconpln.mims.data.local.database.TPenerimaanDetailUlpDao.Properties.NoRepackaging.eq(
                    noRepackaging
                )
            ).list()
        ...
        if (transPenerimaanDetails.isEmpty()) {
            ...
            if (size > 0) {
                val items =
                    arrayOfNulls<dev.iconpln.mims.data.local.database.TTransPenerimaanDetailUlp>(
                        size
                    )
                var item: dev.iconpln.mims.data.local.database.TTransPenerimaanDetailUlp
                for ((i, model) in penerimaanDetails.withIndex()) {
                    item =
                        dev.iconpln.mims.data.local.database.TTransPenerimaanDetailUlp()
    ...
    private fun insertData() {
        ...
        val penerimaanUlps = daoSession.tPenerimaanUlpDao.queryBuilder()
            .where(
                dev.iconpln.mims.data.local.database.TPenerimaanUlpDao.Properties.StatusPemeriksaan.notEq(
                    "SUDAH DIPERIKSA"
                )
            )
            .where(
                dev.iconpln.mims.data.local.database.TPenerimaanUlpDao.Properties.StatusPenerimaan.notEq(
                    "DITERIMA"
                )
            )
            .list()
        ...
        if (transPenerimaanUlp.isEmpty()) {
            ...
            if (size > 0) {
                val items =
                    arrayOfNulls<dev.iconpln.mims.data.local.database.TTransPenerimaanUlp>(size)
                var item: dev.iconpln.mims.data.local.database.TTransPenerimaanUlp
                for ((i, model) in penerimaanUlps.withIndex()) {
                    item = dev.iconpln.mims.data.local.database.TTransPenerimaanUlp()
